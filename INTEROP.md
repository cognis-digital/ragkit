...
How **ragkit** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `ragkit` lives in the **Dataset / catalog** cluster.
...
| from | relation | to |
|---|---|---|
| `ragkit` | JSON findings compose with | [`awesome-cognis`](https://github.com/cognis-digital/awesome-cognis), [`awesome-drone-warfare-osint`](https://github.com/cognis-digital/awesome-drone-warfare-osint) |
| `ragkit` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `ragkit` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `ragkit` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the dataset / catalog cluster.** `ragkit` output feeds the next tool:
```bash
ragkit ... --format json > out.json          # this tool's findings (see `ragkit --help`)
...
ragkit ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
ragkit ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
ragkit ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `ragkit`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `ragkit` slots
