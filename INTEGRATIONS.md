...
**ragkit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `ragkit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
ragkit ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
ragkit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `ragkit` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="ragkit") for rec in records]   # records = your JSON output
