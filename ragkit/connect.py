"""Native cognis-connect emit for ragkit — forward findings to any platform.

Maps ragkit's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ragkit ... --format json | ragkit-emit --to stix
    ragkit-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out['title'] = rec.get('content', '').split('\n')[0]
        out['severity'] = 'medium'
        out['type'] = 'finding'
        out['description'] = rec.get('content', '')
